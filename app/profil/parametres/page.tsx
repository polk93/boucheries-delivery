  ...
  return (
    <div className="min-h-screen bg-creme" style={{ paddingBottom: 72 }}>
      ...
      <div className="px-4 mt-4 space-y-4 max-w-lg mx-auto">
        {sections.map(sec => (
          <div key={sec.titre}>
            <p className="text-[11px] font-bold text-white uppercase tracking-wider mb-1.5 px-1">{sec.titre}</p>
              ...
              {sec.items.map((item, i) => (
                <button key={item.label}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-white truncate">{item.sub}</p>
                  </div>
        ...
        {!user ? (
          ...
          <div className="bg-white rounded-2xl p-4 shadow-sm border-2 border-rouge-vif">
            ...
            <p className="text-xs text-white text-center mb-4">Vous devrez vous reconnecter pour passer commande.</p>
    ...
    <PageWrapper title="📍 Mes adresses" onBack={onBack}>
      <div className="space-y-3">
        {adresses.length === 0 && !ajoutOpen && (
          <div className="bg-white rounded-2xl p-5 text-center text-white shadow-sm">
        ...
        {ajoutOpen ? (
          ...
          <button className="w-full bg-white border-2 border-dashed border-gray-200 rounded-2xl py-4 text-sm font-semibold text-white font-sans"
...
function FavorisSection({ onBack }: { onBack: () => void }) {
  ...
  return (
    <PageWrapper title="❤️ Boucheries favorites" onBack={onBack}>
      {favoris.length === 0
        ? <div className="text-center py-12 text-white"><span className="text-4xl block mb-3">❤️</span><p className="text-sm">Aucune boucherie favorite pour l'instant.</p></div>
        : favoris.map(f => (
  ...
  return (
    <PageWrapper title="📦 Mes commandes" onBack={onBack}>
      <div className="space-y-4">
        ...
        {filtered.length === 0
          ? <div className="text-center py-12 text-white"><span className="text-4xl block mb-3">📦</span><p className="text-sm">Aucune commande pour l'instant.</p><button className="mt-4 bg-brun text-white px-5 py-2 rounded-xl text-sm font-bold font-sans" onClick={() => router.push('/')}>Découvrir les boucheries</button></div>
          : filtered.map(o => (
            <div key={o.id} className="bg-white rounded-2xl shadow-sm overflow-hidden">
              <div className="px-4 py-3 border-b border-gris-bd flex justify-between items-center">
                <div><span className="font-black text-brun text-sm">{o.id}</span><span className="text-white text-xs ml-2">{new Date(o.date).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long' })}</span></div>
              ...
              <div className="px-4 py-3 border-b border-gris-bd">
                ...
                {o.status === 'delivery' && <div className="bg-or-pale border border-or/20 rounded-xl p-2.5 mt-2 flex items-center gap-2"><span className="text-lg">🛵</span><div><p className="text-xs font-bold text-brun">Votre livreur est en route</p><p className="text-[10px] text-white">Arrivée estimée dans ~8 min</p></div></div>}
                {o.status === 'ready' && <div className="bg-green-50 border border-green-200 rounded-xl p-2.5 mt-2 flex items-center gap-2"><span className="text-lg">✅</span><div><p className="text-xs font-bold text-green-700">Votre commande est prête !</p><p className="text-[10px] text-white">Présentez le {o.id} en caisse</p></div></div>}
              </div>
              <div className="px-4 py-3 flex justify-between items-center">
                <div><p className="font-bold text-brun text-sm">{o.total.toFixed(2)} €</p><p className="text-xs text-white">dont {o.frais.toFixed(2)} € livraison</p></div>
                <div className="flex gap-2">
...
function AvisSection({ onBack }: { onBack: () => void }) {
  ...
  return (
    <PageWrapper title="⭐ Mes avis" onBack={onBack}>
      {avis.length === 0
        ? <div className="text-center py-12 text-white"><span className="text-4xl block mb-3">⭐</span><p className="text-sm">Vous n'avez pas encore laissé d'avis.</p></div>
        : <div className="space-y-4">
            {avis.map(a => (
              <div key={a.id} className="bg-white rounded-2xl p-4 shadow-sm">
                <div className="flex justify-between items-start mb-1">
                  <div><p className="font-bold text-brun text-sm">{a.boucherie}</p><p className="text-xs text-white">{a.produit}</p></div>
                  <div className="flex items-center gap-2">
        ...
        {cartes.length === 0
          ...
          : cartes.map(c => (
            <div key={c.id} className="bg-white rounded-2xl p-4 shadow-sm flex items-center gap-3">
              ...
              <div className="flex-1"><p className="font-bold text-brun text-sm">{c.type} •••• {c.last4}</p><p className="text-xs text-white">Expire {c.expiry}</p></div>
          ...
          {faqs.map((faq, i) => (
            <div key={i} className={i < faqs.length - 1 ? 'border-b border-gris-bd' : ''}>
              <button className="w-full flex items-center justify-between gap-3 px-4 py-3.5 text-left font-sans" onClick={() => setOpen(open === i ? null : i)}>
                ...
                <span className={`text-white text-base flex-shrink-0 transition-transform ${open === i ? 'rotate-180' : ''}`}>▾</span>
              </button>
...
function ContactSection({ onBack }: { onBack: () => void }) {
  ...
  if (sent) return (
    <PageWrapper title="✉️ Nous contacter" onBack={onBack}>
      <div className="text-center py-14"><span className="text-6xl block mb-4">✅</span><h2 className="font-serif text-xl font-bold text-brun mb-2">Message envoyé !</h2><p className="text-white text-sm">Notre équipe vous répondra sous 2h.</p></div>
    </PageWrapper>
  )
  return (
    <PageWrapper title="✉️ Nous contacter" onBack={onBack}>
  ...
  return (
    <div>
      <div className="flex items-start justify-between mb-1">
        <div><label className="text-xs font-bold text-brun">{label} {required && <span className="text-rouge-vif">*</span>}</label><p className="text-[10px] text-white">{sublabel}</p></div>
      ...
      {file ? (
        <div className="flex items-center gap-2 bg-green-50 border border-green-200 rounded-xl px-3 py-2.5">
          ...
          <button className="text-white text-xs font-sans" onClick={() => onChange(null)}>✕</button>
        </div>
      ) : (
        <label className="flex items-center gap-2 border-2 border-dashed border-gray-200 rounded-xl px-3 py-3 cursor-pointer hover:border-brun transition-all">
          ...
          <span className="text-xs text-white font-semibold">Choisir un fichier (PDF, JPG, PNG)</span>
      ...
      <div className="space-y-4">
        ...
        {sacIsotherme === null && (
          <div className="bg-white rounded-2xl p-4 shadow-sm space-y-3">
            ...
            <p className="text-xs text-white">Obligatoire pour maintenir la chaîne du froid lors des livraisons.</p>
            <div className="flex gap-3">
              <button
        ...
        {sacIsotherme === 'non' && (
          <div className="space-y-3">
            <div className="bg-or-pale border border-or/20 rounded-xl p-4 space-y-2">
              ...
              <p className="text-xs text-white text-center">Environ 15-20€ · Livraison en 1-2 jours</p>
            </div>
            <button className="w-full text-xs text-white font-sans py-2"
