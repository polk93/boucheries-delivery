      ...
      <div className="bg-white rounded-t-3xl w-full max-w-lg max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center px-5 py-4 border-b border-gris-bd sticky top-0 bg-white">
          <div>
            ...
            <p className="text-base text-gray-400">{item.prix.toFixed(2)} € / unité</p>
          </div>
        ...
        <div className="p-5 space-y-4">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {produit.decoupes.map(d => (
                  <button key={d} className={`px-3 py-1.5 rounded-full border text-base font-semibold font-sans ${decoupe === d ? 'bg-brun text-white border-brun' : 'border-gray-200 text-gray-500'}`}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {produit.preparation.map(pr => (
                  <button key={pr} className={`px-3 py-1.5 rounded-full border text-base font-semibold font-sans ${preparation === pr ? 'bg-brun text-white border-brun' : 'border-gray-200 text-gray-500'}`}
          ...
          <div>
            <p className="text-base font-bold text-brun mb-2">📝 Note boucher</p>
...
export default function PaiementPage() {
    ...
    <div className="min-h-screen bg-creme" style={{ paddingBottom: 40 }}>
      ...
      <div className="max-w-lg mx-auto px-4 py-5 space-y-4">
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm">
          <p className="text-base font-bold text-gray-400 uppercase tracking-wider mb-3">Votre commande</p>
          {items.map(item => (
        ...
        <p className="text-base font-bold text-gray-400 uppercase tracking-wider px-1">Comment récupérer votre commande ?</p>
        ...
        <button className="w-full bg-white rounded-2xl p-5 shadow-sm border-2 border-transparent hover:border-brun transition-all text-left active:scale-[.98]"
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-base text-gray-400 mt-0.5 leading-relaxed">Commandez, récupérez en boutique. Prêt en 20 min.</p>
              {boucherie && <p className="text-base text-or font-semibold mt-1">📍 {boucherie.nom}</p>}
            </div>
        ...
        <button className="w-full bg-white rounded-2xl p-5 shadow-sm border-2 border-transparent hover:border-brun transition-all text-left active:scale-[.98]"
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-base text-gray-400 mt-0.5 leading-relaxed">Livré chez vous en moins de 45 min, chaîne du froid garantie.</p>
              <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                <span className="text-base font-bold text-rouge-vif bg-rouge-pale px-2 py-0.5 rounded-full">
        ...
        <div className="bg-or-pale border border-or/20 rounded-xl p-3 flex gap-2 items-start">
          ...
          <div>
            <p className="text-base font-bold text-brun">Livraison assurée par Stuart</p>
  ...
  return (
    <div className="min-h-screen bg-creme" style={{ paddingBottom: 40 }}>
      ...
      {/* Badge info */}
      <div className={`px-4 py-2 text-center text-base font-bold ${mode === 'click_collect' ? 'bg-green-50 text-green-700' : 'bg-or-pale text-brun-clair'}`}>
        {mode === 'click_collect'
      ...
      <div className="flex max-w-lg mx-auto px-4 pt-4 mb-4">
        {stepLabels.map((s, i) => (
          <div key={i} className="flex-1 flex flex-col items-center gap-1">
            <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center text-base transition-all ${i < step ? 'bg-green-100 border-green-400 text-green-600' : i === step ? 'bg-or-pale border-or text-brun' : 'bg-white border-gray-200 text-gray-300'}`}>
              {i < step ? '✓' : stepIcons[i]}
            </div>
      ...
      <div className="max-w-lg mx-auto px-4 space-y-4">
            ...
            {items.map(item => (
              <div key={item.cart_key} className="flex justify-between items-start py-2.5 border-b border-gris-bd">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <span className="text-sm font-bold text-brun">{(item.prix * item.quantite).toFixed(2)} €</span>
                  <button className="text-gray-300 text-base" onClick={() => setEditItem(item)}>✏️</button>
                </div>
              </div>
            ))}
            <div className="mt-3 space-y-1.5">
              <div className="flex justify-between text-base text-gray-400"><span>Sous-total</span><span>{sousTotal().toFixed(2)} €</span></div>
              <div className="flex justify-between text-base text-gray-400">
                <span>Livraison''</span>
                <span className={frais === 0 ? 'text-green-600 font-bold' : ''}>{frais === 0 ? 'Offerte ✓' : `${frais.toFixed(2)} €`}</span>
              </div>
              {pourboireVal > 0 && (
                <div className="flex justify-between text-base text-gray-400"><span>Pourboire livreur 🙏</span><span>{pourboireVal.toFixed(2)} €</span></div>
              )}
        ...
        {mode === 'livraison' && step === 1 && (
          <div className="bg-white rounded-2xl p-4 shadow-sm space-y-3">
            ...
            <div>
              <label className="text-base font-bold text-brun block mb-1">Adresse</label>
              <input className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm font-sans outline-none focus:border-brun"
        ...
        {mode === 'click_collect' && step === 1 && (
          <div className="bg-white rounded-2xl p-4 shadow-sm space-y-4">
            ...
            {boucherie && (
              <div className="bg-or-pale border border-or/20 rounded-xl p-3 flex gap-3">
                ...
                <div>
                  ...
                  <p className="text-base text-gray-400 mt-0.5">Commande prête à l'heure choisie · Minimum 20 min</p>
                  <p className="text-base text-green-600 font-semibold mt-0.5">✅ Retrait gratuit</p>
                </div>
              </div>
            )}
            ...
            {/* Roue de sélection */}
            <div>
              <p className="text-base font-bold text-gray-400 uppercase tracking-wider mb-3 text-center">Choisissez votre heure</p>

              {creneaux.length === 0 ? (
                <div className="text-center py-6 text-gray-400">
                  <span className="text-3xl block mb-2">😴</span>
                  <p className="text-sm">La boucherie est fermée pour aujourd'hui.</p>
                  <p className="text-base mt-1">Revenez demain dès l'ouverture.</p>
                </div>
              ) : (
                      ...
                      <div key={i}
                        ...
                        <div className="text-center">
                          ...
                          {i === creneauCC && c.minutes <= 30 && (
                            <p className="text-base text-or font-semibold">Dans ~{c.minutes} min</p>
                          )}
                        </div>
                      </div>
                    ))}
            ...
            {creneaux.length > 0 && (
              <div className="bg-creme rounded-xl p-3 text-center">
                <p className="text-base text-gray-400 mb-0.5">Heure sélectionnée</p>
                <p className="font-serif text-2xl font-black text-brun">{creneaux[creneauCC]?.heure}</p>
                <p className="text-base text-green-600 font-semibold mt-0.5">
                  Commande prête dans ~{creneaux[creneauCC]?.minutes} min
                </p>
              </div>
            )}

            <div className="bg-creme rounded-xl p-3 text-base text-gray-500 leading-relaxed">
            ...
            {mode === 'livraison' && (
              <div className="bg-white rounded-2xl p-4 shadow-sm">
                ...
                <div className="flex gap-2 flex-wrap mb-3">
                  {POURBOIRES.map(p => (
                    <button key={p.val}
                      className={`flex-1 min-w-[52px] py-2 rounded-xl border-2 text-base font-bold font-sans transition-all ${pourboire === p.val && !pourboireCustom ? 'bg-brun text-white border-brun' : 'border-gray-200 text-gray-500 hover:border-brun'}`}
            ...
            <div className="bg-white rounded-2xl p-4 shadow-sm">
              ...
              <div className={`flex items-center gap-3 p-3 rounded-xl mb-4 ${mode === 'click_collect' ? 'bg-green-50 border border-green-200' : 'bg-or-pale border border-or/20'}`}>
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-base font-bold text-brun">
                    {mode === 'click_collect'
              ...
              {/* Détail total */}
              <div className="space-y-1 mb-4 text-base text-gray-400">
              ...
              <div className="bg-gradient-to-br from-brun to-brun-clair rounded-2xl p-4 text-white mb-4">
                ...
                <div className="flex justify-between text-base opacity-75">
              ...
              <div className="space-y-3">
                <div>
                  <label className="text-base font-bold text-brun block mb-1">Numéro de carte</label>
                ...
                <div>
                  <label className="text-base font-bold text-brun block mb-1">Titulaire</label>
                  <input className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm font-sans outline-none focus:border-brun"
                ...
                <p className="text-base text-gray-400 text-center">🔒 Paiement sécurisé SSL</p>
              </div>
            </div>
