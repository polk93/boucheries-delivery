...
// Navigation bas de page CÔTÉ BOUCHER uniquement
// Affiche : Commandes · Produits · Stocks
export default function BottomNavBoucher({ currentTab, onTabChange }: {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white border-t-2 border-or z-20 shadow-[0_-4px_20px_rgba(61,32,18,.15)]">
      {/* Indicateur rôle boucher */}
      <div className="bg-brun py-1 text-center">
        <span className="text-[10px] font-bold text-or tracking-wider">🔪 ESPACE BOUCHER</span>
      </div>
      <div className="flex">
        {tabs.map(t => (
          <button key={t.id}
            className={`flex-1 flex flex-col items-center justify-center py-2.5 gap-1 border-none bg-transparent cursor-pointer transition-colors ${currentTab === t.id ? 'text-or' : 'text-gray-400'}`}
