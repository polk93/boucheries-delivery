  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-[60] flex items-center justify-center p-5" onClick={onClose}>
      <div className="bg-white rounded-2xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
        ...
        <p className="text-base text-gray-400 mb-5">Personnalisez votre découpe avant d'ajouter au panier</p>

        {/* Découpe */}
        {p.decoupes?.length > 0 && (
          <div className="mb-4">
            <p className="text-base font-bold text-brun mb-2">🔪 Découpe souhaitée</p>
            <div className="flex flex-wrap gap-2">
              {p.decoupes.map(d => (
                <button key={d}
                  className={`px-3 py-1.5 rounded-full border text-base font-semibold transition-all cursor-pointer ${decoupe === d ? 'bg-brun text-white border-brun' : 'bg-white text-gray-500 border-gray-200 hover:border-brun'}`}
          ...
          <div className="mb-4">
            ...
            <div className="flex flex-wrap gap-2">
              {p.preparation.map(pr => (
                <button key={pr}
                  className={`px-3 py-1.5 rounded-full border text-base font-semibold transition-all cursor-pointer ${preparation === pr ? 'bg-brun text-white border-brun' : 'bg-white text-gray-500 border-gray-200 hover:border-brun'}`}
        ...
        {/* Note */}
        <div className="mb-5">
          <p className="text-base font-bold text-brun mb-2">📝 Note pour le boucher (optionnel)</p>
          <textarea
